...
public class MapBuilder : MonoBehaviour
{
    ...
    public float Latitude = 31.0461f;
    public float Longitude = 34.8516f;
    ...
    public void ShowMap()
    {
        _centerTile = new TileInfo(new WorldCoordinate { Lat = Latitude, Lon = Longitude }, ZoomLevel, MapTileSize);
        LoadTiles();
    }
    ...
    private void LoadTiles(bool forceReload = false)
    {
        ...
        for (var x = -size; x <= size; x++)
        {
            for (var y = -size; y <= size; y++)
            {
                ...
                tile.SetTileData(
                     new TileInfo(_centerTile.X - x, _centerTile.Y + y, ZoomLevel, MapTileSize),
                     forceReload);
